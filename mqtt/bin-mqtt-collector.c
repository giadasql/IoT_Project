...
static void mqtt_event(struct mqtt_connection *m, mqtt_event_t event, void *data) {
  switch (event) {
    ...
    case MQTT_EVENT_PUBLISH: {
      ...
      mqtt_message_t *msg = data;

      if (strcmp(msg->topic, CONFIG_RESPONSE_TOPIC) == 0) {
        char received_collector_id[64];
        char received_coap_address[64];

        if (sscanf((char *)msg->payload, "{\"collector_id\":\"%63[^\"]\",\"coap_server_address\":\"%63[^\"]\"}",
...
static void client_callback_lid_state(coap_message_t *response) {
  ...
  if (response) {
    ...
    snprintf(pub_msg, sizeof(pub_msg), "{\"lid_state\":\"%s\"}", (char *)payload);
...
PROCESS_THREAD(coap_to_mqtt_process, ev, data) {
  ...
  while (1) {
    ...
    if ((ev == PROCESS_EVENT_TIMER && data == &periodic_timer) || ev == PROCESS_EVENT_POLL) {
      ...
      } else if (state == STATE_CONFIG_REQUEST) {
      ...
      } else if (state == STATE_CONFIG_RECEIVED) {
